...
export interface MultiWorkflowExecution {
  ...
  technologies: string[];
  // Clarification response — returned when the command is ambiguous
  clarification?: boolean;
  question?: string;
  options?: WorkflowPreview[];
}
...
interface WorkflowStore {
  ...
  getRunningWorkflowsForRepository: (
    repository: string
  ) => Array<{ workflowName: string; runId: string; startTime: Date }>;
  getWorkflowDuration: (
    repository: string,
    workflowName: string,
    githubToken?: string
...
export const useWorkflowStore = create<WorkflowStore>()(
  persist(
    (set, get) => ({
      ...
      previewWorkflows: async (command: string) => {
        try {
          ...
          if (
            ...
            data.response.includes('"workflows"')
          ) {
            try {
      ...
      triggerMultipleWorkflows: async (
        workflows: WorkflowPreview[],
        token?: string,
        branch?: string
      ) => {
        ...
        for (const workflow of workflows) {
          try {
            ...
            const result = await get().triggerWorkflow(
      ...
      fetchWorkflowLogs: async (runId: string, token?: string, repository?: string) => {
        try {
          ...
          const response = await fetch(`/api/workflow-logs?runId=${runId}${repoParam}`, {
            headers,
          });
          if (!response.ok) {
            const errorData = await response.json();
      ...
      startPollingLogs: (runId: string, token?: string, repository?: string) => {
        ...
        const pollInterval = setInterval(async () => {
          ...
          console.log(
            "🔍 Polling logs - Current status:",
            updatedLogs?.run.status,
            "Conclusion:",
            updatedLogs?.run.conclusion,
            "Has reportArtifact:",
            !!updatedLogs?.reportArtifact
          );

          // Check if workflow is finished (including external cancellations)
          if (
            updatedLogs?.run.status === "completed" ||
            updatedLogs?.run.status === "failed" ||
            updatedLogs?.run.status === "cancelled"
          ) {
            ...
            if (!finalCheckScheduled) {
              ...
              console.log(
                "🛑 Workflow finished - scheduling final check for artifacts in 3 seconds..."
              );
              ...
              setTimeout(async () => {
                ...
                console.log(
                  "✅ Final check complete - Has reportArtifact:",
                  !!finalLogs?.reportArtifact,
                  "isViewable:",
                  finalLogs?.reportArtifact?.isViewable
                );
          ...
          if (updatedLogs?.logs) {
            const allJobsCompleted = updatedLogs.logs.every(
              (log: any) =>
            ...
            if (allJobsCompleted) {
              ...
              if (!finalCheckScheduled) {
                finalCheckScheduled = true;
                console.log(
                  "🛑 All jobs completed - scheduling final check for artifacts in 3 seconds..."
                );

                setTimeout(async () => {
                  console.log(
                    "🔍 Final check - fetching logs one more time to get final artifacts"
                  );
                  await get().fetchWorkflowLogs(runId, token, repository);
                  const { currentLogs: finalLogs } = get();
                  console.log(
      ...
      startPollingMultipleLogs: (runIds: string[], token?: string, repositories?: string[]) => {
        ...
        const pollInterval = setInterval(async () => {
          ...
          validLogs.forEach((log) => {
            ...
            const allJobsCompleted = log.logs
              ? log.logs.every(
                  (jobLog: any) =>
            ...
            if (
              workflowFinished &&
              allJobsCompleted &&
              !finishedRunIds.has(log.run.id.toString())
            ) {
          ...
          if (allWorkflowsFinished) {
            ...
            if (!finalCheckScheduled) {
              finalCheckScheduled = true;
              console.log(
                "🛑 All workflows finished - scheduling final check for artifacts in 3 seconds..."
              );

              setTimeout(async () => {
                console.log(
                  "🔍 Final check - fetching logs one more time to get final artifacts for all workflows"
                );
                ...
                validFinalLogs.forEach((log) => {
                  console.log(
                    `  - Run ${log.run.id}: Has reportArtifact: ${!!log.reportArtifact}, isViewable: ${log.reportArtifact?.isViewable}`
                  );
                });
      ...
      getWorkflowDuration: async (
        ...
        try {
          ...
          const response = await fetch(
            `/api/workflow-durations?repository=${encodeURIComponent(repository)}`,
            {
              headers,
            }
          );
          ...
          const workflowDuration = data.durations?.find(
            (d: any) => d.workflowName === workflowName
          );
