...
export default function WorkflowStatus({ githubToken }: WorkflowStatusProps) {
  ...
  useEffect(() => {
    ...
    const repositoryDataNames = ["pw-cookunity-automation", "wdio-cookunity-automation"];
  ...
  const getEnvironmentBadge = (environment: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors[environment as keyof typeof colors] || "bg-gray-100 text-gray-800"}`}
      >
  ...
  const extractEnvironmentFromName = (workflowName: string | any): string => {
    ...
    const nameStr =
      typeof workflowName === "string" ? workflowName : workflowName?.name || String(workflowName);
    const name = nameStr.toLowerCase();
    if (name.includes("prod")) return "prod";
    if (name.includes("qa")) return "qa";
    if (name.includes("staging")) return "staging";
    if (name.includes("ios") || name.includes("android") || name.includes("maestro"))
      return "mobile";
  ...
  const isWorkflowRunningFromTodd = (workflowName: string, repository: string): boolean => {
    return runningWorkflowsFromTodd.some(
      (w) => w.workflowName === workflowName && w.repository === repository
    );
  };
      ...
      if (result && result.runId) {
        ...
        setWorkflowStates((prev) => ({
          ...
          [workflowId]: {
            ...
            runId: result.runId,
            htmlUrl:
              result.htmlUrl || `https://github.com/${repository}/actions/runs/${result.runId}`,
            startTime: new Date(),
          },
        }));
  ...
  if (error) {
    ...
    const isAuthError =
      error.includes("401") || error.includes("404") || error.includes("Authentication Error");
    ...
    return (
      <div className="w-full max-w-none mx-auto px-8 sm:px-12 lg:px-16 xl:px-20 pt-4 sm:pt-8 lg:pt-20">
        ...
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          <h1
            className="text-3xl sm:text-4xl lg:text-5xl font-mono mb-3 tracking-wide"
            style={{ color: "#344055" }}
          >
            Testing Workflows
          </h1>
  ...
  return (
    <div className="w-full max-w-none mx-auto px-8 sm:px-12 lg:px-16 xl:px-20 pt-4 sm:pt-8 lg:pt-20">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 pb-32 items-start">
            ...
            >
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-8 h-8 rounded-lg bg-${repo.color}-500/20 flex items-center justify-center`}
                  >
                    <IconComponent className={`w-5 h-5 text-${repo.color}-400`} />
                  </div>
                  <div>
                    <div className="flex items-center space-x-2">
                      <h3 className="font-medium font-mono" style={{ color: "#344055" }}>
                        {repo.name}
                      </h3>
                      {(() => {
                    ...
                    <p className="text-sm font-mono" style={{ color: "#6B7280" }}>
                      {repo.technology} • {repo.workflows.length} workflows
                    </p>
                  </div>
                </div>
                {isExpanded ? (
                    ...
                    return (
                      <>
                        {workflowsToShow.map((workflow: any) => {
                          ...
                          const hasSchedule =
                            workflow.hasSchedule !== undefined
                              ? workflow.hasSchedule
                              : isScheduledWorkflow(workflowName);
                          const canExecute =
                            workflow.canExecute !== undefined
                              ? workflow.canExecute
                          ...
                          return (
                            <div
                              ...
                              className={`border rounded-xl p-4 transition-all duration-200 bg-white/20 hover:bg-white/30 hover:shadow-lg ${getWorkflowStateColor(workflowId)} ${
                                isRunning ? "cursor-default opacity-75" : "cursor-pointer"
                              }`}
                              onClick={() => {
                                if (!isRunning) {
                            ...
                            >
                                  ...
                                  <div className="flex items-center space-x-2 flex-wrap gap-1.5">
                                    ...
                                    {workflowState?.reportUrl && (
                                      ...
                                      >
                                        ...
                                        {workflowState.reportIsViewable
                                          ? "View Report"
                              ...
                              {isWorkflowRunningFromTodd(workflowName, repo.name) &&
                                state === "idle" &&
                                (() => {
                                  const runningWorkflow = runningWorkflowsFromTodd.find(
                                    (w) =>
