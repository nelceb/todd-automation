...
interface ChatInterfaceProps {
  ...
  onNavigateToGenerator?: (skillType: "subscription" | "landing" | "mobile" | null) => void;
}
...
const extractTestSummary = (logs: string) => {
  ...
  const summaryJsonPattern =
    /"total":\s*(\d+).*"passed":\s*(\d+).*"failed":\s*(\d+).*"skipped":\s*(\d+)/g;
  ...
  if (hasWorkers) {
    ...
    if (mavenMatches.length > 0) {
      ...
      console.log("🔍 Using final Maven summary for multi-worker:", {
        passed,
        failed,
        skipped,
        errors,
...
export default function ChatInterface({
  ...
  const executeSubmit = useCallback(
    async (userMessage: string) => {
      ...
      // ── Detect "create test" intent before workflow execution ──────────────────
      if (onNavigateToGenerator) {
        const lower = userMessage.toLowerCase();
        const isCreateIntent =
          /\b(create|generate|write|add|make)\b.{0,30}\btest\b|\btest\b.{0,30}\b(create|generate|write|add)\b/i.test(
            lower
          );

        if (isCreateIntent) {
          const isMobile = /\b(mobile|ios|iphone|wdio|webdriverio|appium)\b/i.test(lower);
          const isWeb =
            /\b(web|playwright|subscription|landing|coreux|core ux|page|browser|desktop)\b/i.test(
              lower
            );
          const isBoth = isMobile && isWeb;

          let skillType: "subscription" | "landing" | "mobile" | null = null;
          let description = "";

          if (isBoth) {
            // For both, open the type-selection screen (step 1)
            description = "Opening the test generator — select the type you want to start with.";
          } else if (isMobile) {
            skillType = "mobile";
            description = "Opening the Mobile test generator (WDIO + Appium for iOS).";
          } else if (isWeb) {
            const isLanding =
              /\b(landing|home|lp|growth|signup|sign.?up|strapi|contentful|gift.?card|segment)\b/i.test(
                lower
              );
            skillType = isLanding ? "landing" : "subscription";
            description = `Opening the ${skillType === "landing" ? "Landing" : "Subscription"} test generator (Playwright).`;
          } else {
            // Generic "create test" with no platform hint → show type selection
            description = "Opening the test generator — select the type you want to start with.";
          }
          ...
          // null = open the generator at step 1 (type selection); specific type = skip to step 2
          onNavigateToGenerator(skillType);
          return;
        }
      }
      // ───────────────────────────────────────────────────────────────────────────
      ...
      try {
        ...
        if (!githubToken) {
          setMessages((prev) => [
            ...prev,
            {
              ...
              type: "assistant",
              content:
                'GitHub authentication is required to execute workflows. Please connect to GitHub first using the "Connect to GitHub" button in the header.',
              timestamp: new Date(),
            },
          ]);
          setIsLoading(false);
        ...
        if (
          ...
          for (const match of allMatches) {
            ...
            if (
              environmentNames.includes(potentialBranchLower) ||
          ...
          for (const workflow of preview.workflows) {
            try {
              ...
              const repoName = workflow.repository
                ? workflow.repository.split("/").pop()
                : "pw-cookunity-automation";
              console.log(
                `🚀 Triggering workflow: "${workflow.workflowName}" in repository: "${repoName}"`
              );
          ...
          if (runIds.length > 0) {
            const repositories = results.map((result) => {
              const repoName = result.workflow?.repository
                ? result.workflow.repository.split("/").pop()
                : "pw-cookunity-automation";
              return repoName;
            });
  ...
  const handleSubmitDirectly = useCallback(
  ...
  return (
    <div
      className="w-full px-2 sm:px-4 lg:px-6 xl:px-8 flex flex-col"
    ...
    >
          ...
          >
            ...
            >
              <div className="relative flex items-center justify-center mb-4">
                ...
                <h1
                  className="text-8xl font-mono tracking-wide relative z-10"
                  style={{ color: "#344055" }}
                >
                  TODD
                </h1>
              ...
              <div
                className="mb-6 w-full max-w-4xl mx-auto px-4 text-center"
                style={{ color: "#344055" }}
              >
                ...
                <div className="max-w-4xl w-full space-y-3">
                        ...
                        <div className="flex items-center space-x-3 mb-1">
                          <div
                            className={`w-2 h-2 rounded-full ${
                              message.type === "user" ? "bg-airforce-400" : "bg-asparagus-400"
                            }`}
                          ></div>
                  ...
                  {isLoading && (
                    ...
                    >
                      <div className="flex-shrink-0 text-xs text-gray-700 font-mono mt-1 w-[60px] sm:w-[120px] text-right">
                        {typeof window !== "undefined"
                          ? formatDistanceToNow(new Date(), { addSuffix: true, locale: enUS })
                          : "now"}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-3 mb-1">
              ...
              {messages.length > 0 && messages[messages.length - 1]?.workflowPreview && (
                <div className="flex justify-center">
                  <div className="max-w-4xl w-full">
                    ...
                    >
                      {messages[messages.length - 1].workflowPreview.workflows.map(
                              ...
                              <div className="text-gray-800 text-sm leading-relaxed font-mono space-y-1 ml-5">
                                <div className="flex items-center space-x-2">
                                  ...
                                  {(() => {
                                    ...
                                    const correspondingLog = multipleLogs.find(
                                      (log) =>
                                    ...
                                    const hasResults =
                                      correspondingLog &&
                                      correspondingLog.logs &&
                                      correspondingLog.logs.length > 0 &&
                                ...
                                {Object.keys(workflow.inputs).length > 0 && (
                                  <div className="text-gray-700 text-xs">
                                    ...
                                    {Object.entries(workflow.inputs).map(
                                      ([key, value], inputIndex) => (
                      ...
                      {(multipleLogs.length > 0
                        ...
                        <div key={index} className="space-y-3">
                                    ...
                                    if (duration) {
                                      return (
                                        <p className="text-xs text-gray-700">
                                          ...
                                          {Math.round(duration.minDurationMinutes)}-
                                          {Math.round(duration.maxDurationMinutes)} min (avg:{" "}
                                          {Math.round(duration.averageDurationMinutes)} min)
                                        </p>
                                      );
                                    }
                                    return null;
                          ...
                          {logs.logs.length > 0 &&
                            (() => {
                              ...
                              if (workflowMessage && workflowMessage.workflowResult) {
                                const matchingResult = workflowMessage.workflowResult.find(
                                  (result: any) => {
                                    if (result.runId && logs.run.id) {
                                      return result.runId.toString() === logs.run.id.toString();
                                    }
                                ...
                                if (
                                  matchingResult &&
                                  matchingResult.workflow &&
                                  matchingResult.workflow.workflowName
                                ) {
                                ...
                                <div className="flex items-start space-x-4 py-2">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center space-x-3 mb-1">
                                      ...
                                      ></div>
                                      <span
                                        className={`text-xs font-medium uppercase tracking-wide w-[80px] ${
                                          summary.failed === 0 && summary.passed > 0
                                    ...
                                    <div
                                      className={`text-sm leading-relaxed font-mono space-y-1 ml-5 ${
                                        summary.failed === 0 && summary.passed > 0
                          ...
                          {(logs.run.status === "completed" ||
                            logs.run.status === "failed" ||
                            logs.run.status === "cancelled") && (
                            <div className="flex items-start space-x-4 py-2">
                              <div className="flex-shrink-0 text-xs text-gray-700 font-mono mt-1 w-[60px] sm:w-[120px] text-right">
                                {typeof window !== "undefined"
                                  ? formatDistanceToNow(
                                      new Date(logs.run.updatedAt || logs.run.createdAt),
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-700/50 flex items-center justify-center space-x-4 flex-wrap gap-3">
                              {logs.reportArtifact && logs.reportArtifact.htmlUrl && (
                                ...
                                >
                                  <span>
                                    📊{" "}
                                    {logs.reportArtifact.isViewable
                                      ? "View Test Report"
                                      : "View Test Report (Download)"}
                                  </span>
                                  <svg
                                    className="w-5 h-5"
                                    fill="none"
                              ...
                              {logs.run.status === "in_progress" &&
                                ...
                                (() => {
                                  ...
                                  if (logs.run.htmlUrl) {
                                    const urlMatch = logs.run.htmlUrl.match(
                                      /github\.com\/([^\/]+)\/([^\/]+)\//
                                    );
                                  ...
                                  if (!repoName) {
                                    ...
                                    if (
                                      ...
                                      workflowPreview.workflows.length > 0
                                    ) {
                                  ...
                                  return (
                                    <button
                                      onClick={() =>
                                        handleCancelWorkflow(logs.run.id.toString(), repoName!)
                                      }
                                      className="inline-flex items-center space-x-2 text-sm sm:text-base font-mono text-white transition-colors px-3 py-2 sm:px-4 sm:py-2 border-2 border-redwood-600 rounded-lg hover:border-redwood-500 bg-redwood-500 hover:bg-redwood-600 shadow-md min-h-[44px]"
                                    >
                                      <svg
                                        className="w-5 h-5"
                                        fill="none"
      ...
      {messages.length > 0 && (
        <div
          className="flex-shrink-0 p-4 pb-8 border-t border-gray-300/30"
          style={{ backgroundColor: "#AED4E6" }}
        >
              ...
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
                <motion.button
                  ...
                  className={`relative transition-colors flex items-center justify-center ${
                    isListening ? "" : "text-gray-700 hover:text-gray-900"
                  }`}
                >
                  <motion.div
                    animate={
