...
const TARGET_REPOS = ["Cook-Unity/pw-cookunity-automation", "Cook-Unity/wdio-cookunity-automation"];
        ...
        try {
          ...
          if (!repoResponse.ok) {
            ...
            if (repoResponse.status === 403 && errorText.includes("rate limit")) {
              console.warn(
                `⚠️ Rate limit reached while fetching repo ${repoName}, returning minimal data`
              );
              return {
            ...
            throw new Error(
              `Failed to fetch repo ${repoName}: ${repoResponse.status} - ${errorText}`
            );
          }
          ...
          while (true) {
            ...
            if (!workflowsResponse.ok) {
              const errorText = await workflowsResponse
                .text()
                .catch(() => workflowsResponse.statusText);
              // Si es rate limit, no lanzar error, devolver datos parciales
              if (workflowsResponse.status === 403 && errorText.includes("rate limit")) {
                console.warn(
                  `⚠️ Rate limit reached while fetching workflows for ${repoName}, using partial data`
                );
                break; // Usar los workflows que ya obtuvimos
              }
              throw new Error(
                `Failed to fetch workflows for ${repoName}: ${workflowsResponse.status} - ${errorText}`
              );
            }
          ...
          console.log(
            `📋 [${repoName}] Total workflows obtenidos (con paginación): ${allWorkflowsFromAPI.length}`
          );
          ...
          console.log(
            `📋 [${repoName}] Active workflows (después de filtro): ${activeWorkflows.length}`
          );

          // Verificar específicamente si qa_us_coreux_regression está en la lista
          const regressionWorkflow = allWorkflowsFromAPI.find(
            (w: any) =>
          ...
          if (regressionWorkflow) {
            console.log(
              `✅ [${repoName}] ENCONTRADO qa_us_coreux_regression en /api/repositories:`,
              {
            ...
            console.log(
              `❌ [${repoName}] NO encontrado qa_us_coreux_regression en /api/repositories`
            );
          }

          // Classify workflows by technology and purpose, and check YAML for schedule and workflow_dispatch
          const classifiedWorkflows = await Promise.all(
            activeWorkflows.map(async (workflow: any) => {
              const classification =
                REPO_CLASSIFICATION[repoName as keyof typeof REPO_CLASSIFICATION];
              ...
              try {
                ...
                if (yamlResponse.ok) {
                  ...
                  if (yamlData.content) {
                    ...
                    isDependabot =
                      workflow.path.includes("dependabot") ||
                      workflow.name.toLowerCase().includes("dependabot");
                  }
                }
              } catch (error) {
                ...
                if (errorMessage.includes("rate limit")) {
                  console.warn(
                    `⚠️ Rate limit while checking YAML for ${workflow.name}, skipping YAML check`
                  );
                } else {
          ...
          return {
            ...
            technology:
              REPO_CLASSIFICATION[repoName as keyof typeof REPO_CLASSIFICATION].technology,
          ...
          if (errorMessage.includes("rate limit")) {
            ...
            if (allWorkflowsFromAPI && allWorkflowsFromAPI.length > 0) {
              const activeWorkflows = allWorkflowsFromAPI.filter((workflow: any) => {
                ...
                if (nameLower.includes("auto test pr") || nameLower.includes("auto-test-pr"))
                  return false;
                return workflow.state === "active";
              });
              ...
              return {
                ...
                workflows: activeWorkflows.map((w: any) => ({
                  ...
                  technology:
                    ...
                    "Unknown",
                  platforms:
                    REPO_CLASSIFICATION[repoName as keyof typeof REPO_CLASSIFICATION]?.platforms ||
                    [],
                  description: w.name,
                  category: categorizeWorkflow(
                    w.name,
                    REPO_CLASSIFICATION[repoName as keyof typeof REPO_CLASSIFICATION]?.technology ||
                      ""
                  ),
