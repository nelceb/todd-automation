...
function generatePlaywrightWhenSteps(when: string, type: string = 'single', selectors: any, title?: string, existingMethods?: any): string {
  ...
  if (whenLower.includes('taps orders tab') || whenLower.includes('navigates to orders') || whenLower.includes('user taps orders tab')) {
    ...
    // For Home/Homepage tests, stay on Home page - use page object methods
    if (titleLower.includes('swimlane') || whenLower.includes('scroll') || whenLower.includes('swimlane')) {
      // For Order Again swimlane, use page object method
      return `// User is already on Home page - scroll to Order Again section using page object method
    ...
    // No action needed - banner is at top of page`
      ...
      // For meals section, use page object method
      return `// User is already on Home page - scroll to meals section using page object method
...
function suggestPageObjectMethods(acceptanceCriteria: AcceptanceCriteria): string[] {
  ...
  if (title.includes('home') || title.includes('homepage')) {
    if (title.includes('swimlane') || then.includes('swimlane')) {
      suggestions.push('homePage.scrollToOrderAgainSection() - Use page object method (no hardcoded selectors)')
      suggestions.push('homePage.isOrderAgainSwimlaneVisible() - Check if Order Again swimlane is visible (page object method)')
    }
    if (title.includes('banner') || then.includes('banner')) {
      suggestions.push('// No scroll needed - banner is at top of page')
      suggestions.push('homePage.isBannerCarouselDisplayed() - Check if banner carousel is displayed (page object method)')
    }
    if (title.includes('meals') || then.includes('meals')) {
      suggestions.push('homePage.scrollToMealsSection() - Use page object method (no hardcoded selectors)')
      suggestions.push('homePage.isMealsSectionVisible() - Check if meals section is visible (page object method)')
    }
  }
