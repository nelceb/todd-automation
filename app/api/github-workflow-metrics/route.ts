...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`📊 Metrics: Total workflows retrieved (with pagination) for ${repo}: ${allWorkflows.length}`)
    console.log(`📊 Metrics: Active workflows (after filter) for ${repo}: ${activeWorkflows.length}`)
    console.log(`📊 Metrics: Processing top ${workflows.length} workflows to avoid rate limits`)
    
    if (activeWorkflows.length > MAX_WORKFLOWS_TO_PROCESS) {
      console.warn(`⚠️ There are ${activeWorkflows.length} active workflows, but only processing ${MAX_WORKFLOWS_TO_PROCESS} to avoid rate limits`)
    }
