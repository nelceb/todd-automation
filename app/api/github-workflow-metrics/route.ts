...
export async function GET(request: NextRequest) {
  try {
    ...
    const activeWorkflows = allWorkflows.filter((workflow: any) => {
      ...
      // Excluir dependabot
      if (nameLower.includes("dependabot") || pathLower.includes("dependabot")) {
        return false;
      }
    ...
    // Only keep workflows that have workflow_dispatch trigger (same filter as Workflows tab)
    const workflowDispatchChecks = await Promise.allSettled(
      activeWorkflows.map(async (workflow: any) => {
        try {
          const yamlResponse = await fetch(
            `https://api.github.com/repos/${repo}/contents/${workflow.path}`,
            {
              headers: {
                Authorization: `token ${token}`,
                Accept: "application/vnd.github.v3+json",
          ...
          if (!yamlResponse.ok) return { workflow, hasDispatch: false };
          const yamlData = await yamlResponse.json();
          const yamlContent = yamlData.content
            ? Buffer.from(yamlData.content, "base64").toString("utf-8")
            : "";
          const hasDispatch = yamlContent.includes("workflow_dispatch");
          return { workflow, hasDispatch };
        } catch {
          return { workflow, hasDispatch: false };
        }
      })
    );

    const workflows = workflowDispatchChecks
      .filter((r) => r.status === "fulfilled" && r.value.hasDispatch)
      .map(
        (r) => (r as PromiseFulfilledResult<{ workflow: any; hasDispatch: boolean }>).value.workflow
      );

    console.log(
      `📊 Metrics: workflows with workflow_dispatch for ${repo}: ${workflows.length} (of ${activeWorkflows.length} active)`
    );
    console.log(
      `📊 Metrics: Total workflows obtenidos (con paginación) para ${repo}: ${allWorkflows.length}`
    );
    console.log(
      `📊 Metrics: Active workflows (después de filtro) para ${repo}: ${workflows.length}`
    );
    ...
    for (const workflow of workflows) {
      try {
        ...
        if (!runsResponse.ok) {
          console.warn(
            `Failed to fetch runs for workflow ${workflow.name}: ${runsResponse.statusText}`
          );
          continue;
        }
        ...
        const toddRuns = runs.filter((run) => {
          if (run.event !== "workflow_dispatch" && run.event !== "repository_dispatch")
            return false;
    ...
    const totalScheduledRuns = workflowMetrics.reduce(
      (sum, w) => sum + (w.trigger_breakdown?.schedule || 0),
      0
    );
    const totalManualRuns = workflowMetrics.reduce(
      (sum, w) => sum + (w.trigger_breakdown?.workflow_dispatch || 0),
      0
    );
    ...
    const mostActiveWorkflow =
      workflowMetrics.length > 0
        ? workflowMetrics.reduce(
            (max, w) => (w.total_runs > max.total_runs ? w : max),
            workflowMetrics[0]
          )
        : null;
