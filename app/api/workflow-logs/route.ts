...
export async function GET(request: NextRequest) {
  try {
      ...
      if (job.status === "completed" || job.status === "in_progress") {
        try {
          ...
          if (logsResponse.ok) {
            ...
            const limitedLogText = logText.length > 500000 ? logText.slice(-500000) : logText;
    ...
    const testJob = jobsData.jobs.find((job: any) => {
      ...
      return (
        testJobNames.some((testName) => jobNameLower.includes(testName)) &&
    ...
    try {
      ...
      if (artifactsResponse.ok) {
        ...
        if (reportArtifact) {
          console.log(
            "✅ Found report artifact:",
            reportArtifact.name,
            "Size:",
            reportArtifact.size_in_bytes
          );
        }
      }
    } catch (error) {
    ...
    for (const patternStr of htmlReportUrlPatterns) {
      ...
      if (matches.length > 0) {
        ...
        if (baseUrl.includes("playwright-report/index.html")) {
          s3HtmlReportUrl = url;
          console.log(
            "✅ Found S3 URL with playwright-report/index.html:",
            s3HtmlReportUrl.substring(0, 100) + "..."
          );
          break;
        }
        ...
        if (
          ...
          !baseUrl.includes("playwright-report")
        ) {
    ...
    if (s3HtmlReportUrl) {
      ...
      console.log(
        "⚠️ GitHub artifact found but no S3 URL - artifact requires download, not viewable"
      );
    ...
    const s3UrlPattern =
      /https:\/\/[^\/]+\.s3\.[^\/]+\/reports\/[^\/]+\/openai_summary\.txt[^\s\n]*/gi;
    ...
    if (s3UrlMatch && s3UrlMatch.length > 0) {
      ...
      try {
        ...
        if (summaryResponse.ok) {
          ...
          console.log(
            "✅ Successfully fetched AI Errors Summary from S3:",
            aiErrorsSummary.length,
            "characters"
          );
        } else {
    ...
    return NextResponse.json({
      ...
      reportArtifact: reportArtifact
        ? {
            ...
            htmlUrl:
              reportArtifact.htmlUrl ||
              `https://github.com/${fullRepoName}/actions/runs/${runId}/artifacts/${reportArtifact.id}`,
