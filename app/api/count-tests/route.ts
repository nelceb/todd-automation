...
export async function GET(request: NextRequest) {
  try {
    ...
    const repositories = [
      ...
      {
        name: 'Cook-Unity/wdio-cookunity-automation',
        framework: 'wdio',
    ...
    // Agregar datos estimados para WDIO si no se obtuvo de API (temporalmente)
    testCounts.push({
...
function isTestFileByPath(filePath: string, basePath: string): boolean {
  ...
  // Selenium: .kt files in src/test directory
