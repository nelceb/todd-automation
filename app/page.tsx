...
import CookUnityTestGenerator from "./components/CookUnityTestGenerator";
import { useWorkflowStore } from "./store/workflowStore";
import { useCookUnityTestStore } from "./store/cookUnityTestStore";
...
export default function Home() {
  const [activeTab, setActiveTab] = useState<
    "chat" | "workflows" | "generator" | "metrics" | "users" | "cu-generator"
  >("chat");
  const [githubToken, setGithubToken] = useState<string>("");
  const [messages, setMessages] = useState<Message[]>([]);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const { workflows, isLoading } = useWorkflowStore();
  const { setSkillType: setCuSkillType, setCurrentStep: setCuStep } = useCookUnityTestStore();

  const handleNavigateToGenerator = (skillType: "subscription" | "landing" | "mobile" | null) => {
    if (skillType) {
      setCuSkillType(skillType); // sets currentStep to 2
    } else {
      setCuStep(1); // reset to type selection
    }
    handleTabChange("cu-generator");
  };
  ...
  const handleTabChange = (
    tab: "chat" | "workflows" | "generator" | "metrics" | "users" | "cu-generator"
  ) => {
    setActiveTab(tab);
    if (tab !== "chat") {
        ...
        <div className="flex flex-col items-center px-4">
          ...
          <div className="hidden md:flex justify-center items-center space-x-4 sm:space-x-8">
            ...
            >
              ...
              <span>Generator</span>
              <span
                className="absolute top-0 right-0 px-0.5 py-0 text-[8px] font-bold rounded-bl rounded-tr-lg leading-tight"
                style={{
            ...
            <button
              onClick={() => handleTabChange("cu-generator")}
              className={`relative flex items-center space-x-1 sm:space-x-2 px-3 py-2 sm:px-4 sm:py-2 border rounded-lg transition-colors font-mono text-sm sm:text-base min-h-[44px] ${
                activeTab === "cu-generator"
                  ? "border-gray-600 bg-gray-600 text-white"
                  : "border-gray-600 hover:border-gray-700"
              }`}
              style={{
                color: activeTab === "cu-generator" ? "white" : "#344055",
                backgroundColor: activeTab === "cu-generator" ? "#4B5563" : "transparent",
              }}
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              ...
              >
                <div className="flex flex-col p-2 space-y-2">
                  ...
                  >
                    ...
                    <span>CU Tests</span>
                    <span className="ml-2 px-1.5 py-0.5 text-[10px] font-bold rounded bg-green-500 text-white leading-tight">
                      NEW
                    </span>
                  </button>
